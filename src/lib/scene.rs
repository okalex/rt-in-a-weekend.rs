...
impl Box3d {
    pub fn new(a: Vec3, b: Vec3, mat: Arc<dyn Material>) -> Scene {
        ...
        let quad1: Arc<dyn Hittable> = Arc::new(Quad::new(Vec3::new(min.x(), min.y(), max.z()), dx, dy, Arc::clone(&mat)));
        let quad2: Arc<dyn Hittable> = Arc::new(Quad::new(Vec3::new(max.x(), min.y(), max.z()), -dz, dy, Arc::clone(&mat)));
        let quad3: Arc<dyn Hittable> = Arc::new(Quad::new(Vec3::new(max.x(), min.y(), min.z()), -dx, dy, Arc::clone(&mat)));
        let quad4: Arc<dyn Hittable> = Arc::new(Quad::new(Vec3::new(min.x(), min.y(), min.z()), dz, dy, Arc::clone(&mat)));
        let quad5: Arc<dyn Hittable> = Arc::new(Quad::new(Vec3::new(min.x(), max.y(), max.z()), dx, -dz, Arc::clone(&mat)));
        let quad6: Arc<dyn Hittable> = Arc::new(Quad::new(Vec3::new(min.x(), min.y(), min.z()), dx, dz, Arc::clone(&mat)));
