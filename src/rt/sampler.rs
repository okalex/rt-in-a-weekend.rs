...
impl Sampler {
    pub fn random(samples_per_pixel: Uint) -> Self {
        Self::Random(RandomSampler::new(samples_per_pixel))
    }

    pub fn stratified(samples_per_pixel: Uint) -> Self {
        Self::Stratified(StratifiedSampler::new(samples_per_pixel))
    }

    pub fn foreach_sample(&self, f: impl FnMut(Vector)) {
        match self {
...
impl RandomSampler {
    fn new(samples_per_pixel: Uint) -> Self {
        Self { samples_per_pixel }
    }

    fn foreach_sample(&self, mut f: impl FnMut(Vector)) {
    ...
    fn sample_square(&self) -> Vector {
        Vector::new(rand() - 0.5, rand() - 0.5, 0.0)
    }

    fn integrate_samples(&self, accumulated_color: Color) -> Color {
        accumulated_color / (self.samples_per_pixel as Float)
    }
}
...
impl StratifiedSampler {
    ...
    fn foreach_sample(&self, mut f: impl FnMut(Vector)) {
        for s_j in 0..self.sqrt_spp as Uint {
            for s_i in 0..self.sqrt_spp as Uint {
    ...
    fn sample_square_stratified(&self, s_i: Uint, s_j: Uint, recip_sqrt_spp: Float) -> Vector {
        let px = ((s_i as Float + rand()) * recip_sqrt_spp) - 0.5;
        let py = ((s_j as Float + rand()) * recip_sqrt_spp) - 0.5;
